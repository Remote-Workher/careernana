-- Allow 'monthly' as a valid plan_key (in addition to existing values).
ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_plan_key_check;
ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_plan_key_check
  CHECK (plan_key IS NULL OR plan_key IN ('trial','monthly','quarterly','yearly'));

-- Quota function: per-plan_key monthly limits.
...
BEGIN
  ...
  -- Per-plan_key monthly limits for new plans.
  -- Monthly (and "trial" alias for monthly): 3 resources/month, unlimited courses.
  -- Quarterly / Yearly: unlimited resources & courses.
  IF _plan_key IN ('monthly', 'trial') THEN
    IF _kind = 'resource' THEN _limit := 3; ELSE _limit := 999999; END IF;
  ELSIF _plan_key IN ('quarterly', 'yearly') THEN
    _limit := 999999;
  ELSE
    -- Legacy tier-based limits (no plan_key set).
  ...
  IF _resource_id IS NOT NULL THEN
    ...
    IF _already_unlocked THEN
      ...
      RETURN jsonb_build_object('allowed', true, 'tier', _tier, 'plan_key', _plan_key, 'used', _used, 'limit', _limit, 'already_unlocked', true);
    END IF;
  END IF;
  ...
  IF _used >= _limit THEN RETURN jsonb_build_object('allowed', false, 'reason', 'monthly_limit_reached', 'tier', _tier, 'plan_key', _plan_key, 'used', _used, 'limit', _limit); END IF;
  IF _kind = 'resource' THEN
  ...
  END IF;
  RETURN jsonb_build_object('allowed', true, 'tier', _tier, 'plan_key', _plan_key, 'used', _used + 1, 'limit', _limit, 'already_unlocked', false);
END $$;
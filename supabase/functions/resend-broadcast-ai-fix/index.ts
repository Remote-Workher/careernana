// Creates a Resend Broadcast for the AI-tools-fixed + webinar announcement.
// Flow:
//   POST { action: "sync" }        -> ensures audience exists, syncs profile emails as contacts
//   POST { action: "create" }      -> creates the broadcast (does NOT send)
//   POST { action: "send" }        -> sends the previously created broadcast now
//   POST { action: "status" }      -> returns audience + contact count + last broadcast id
//
// Auth: requires ?secret=... matching SECRET below (one-off admin tool).
...
const SECRET = 'rwh-broadcast-ai-fix-2026-06'
...
const SUBJECT = 'Resume AI is fixed + webinar tomorrow 🎯'
...
Deno.serve(async (req) => {
  ...
  if (action === 'sync') {
    ...
    const { data: sup } = await supabase.from('suppressed_emails').select('email').limit(10000)
  ...
  if (action === 'create') {
    ...
    const res = await rfetch('/broadcasts', {
      ...
      body: JSON.stringify({
        ...
        subject: SUBJECT,
        html: HTML,
        name: 'AI tools fixed + webinar (Jun 2026)',
      }),
    })

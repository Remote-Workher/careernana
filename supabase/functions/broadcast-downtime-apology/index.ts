// Creates a Resend Broadcast for the downtime apology.
// Actions:
...
//   POST { action: "send", broadcastId } -> sends previously created broadcast
//   POST { action: "status" }      -> audience + contact count + broadcasts
//
// Auth: ?secret=... matching SECRET below.
...
const SECRET = 'rwh-broadcast-downtime-2026-07'
...
const SUBJECT = "We're back up — sorry about the downtime"
  ...
  if (action === 'create') {
    ...
    const res = await rfetch('/broadcasts', {
      ...
      body: JSON.stringify({
        ...
        subject: SUBJECT,
        html: HTML,
        name: 'Downtime apology (Jul 2026)',
      }),
    })

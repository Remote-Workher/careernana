...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(parsed)) skills = parsed.filter((x) => typeof x === "string");
    } catch {
      skills = [];
    }
    skills = Array.from(new Set(skills.map((s) => s.trim()).filter(Boolean))).slice(0, 20);
    return new Response(JSON.stringify({ skills }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    console.error("extract-skills error", e);
    return new Response(JSON.stringify({ error: e instanceof Error ? e.message : String(e), skills: [] }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
});

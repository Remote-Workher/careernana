  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[240px] bg-sidebar flex flex-col z-50">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge && (
                <span className="text-[9px] font-extrabold bg-primary text-primary-foreground px-1.5 py-0.5 rounded-full">
                  {item.badge}
                </span>
              )}
            </NavLink>
        ...
        <div className="bg-sidebar-accent rounded-xl p-4">
          ...
          <div className="w-full h-1.5 bg-sidebar-border rounded-full overflow-hidden mb-3">
            <div
              ...
              style={{ width: `${Math.min((tokens / 50) * 100, 100)}%` }}
            />
          </div>
          <button
            onClick={() => navigate("/dashboard/profile#tokens")}
            className="w-full py-2 rounded-lg text-[12px] font-bold text-primary bg-primary-tint hover:bg-primary-tint/80 transition-colors"
          >
            Top up →
          </button>
        </div>
      ...
      <div className="px-4 py-4 border-t border-sidebar-border flex items-center gap-3">
        <div className="w-8 h-8 rounded-full gradient-primary flex items-center justify-center text-primary-foreground text-[11px] font-extrabold">
          {initials}
        </div>

...
export default function CareerCoach() {
  ...
  if (!open) {
    return (
      ...
      >
        ...
        <span className="text-sm font-semibold">Remote Workher AI</span>
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/60 z-[100] transition-opacity duration-300" onClick={() => setOpen(false)} />
      ...
      <div className="fixed top-0 right-0 z-[101] w-[420px] h-full bg-card shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full gradient-primary flex items-center justify-center text-primary-foreground font-bold text-lg">AI</div>
            <div>
              <p className="text-sm font-bold text-foreground">Remote Workher AI</p>
        ...
        {profileContext && (
          <div className="mx-4 mt-3 px-3.5 py-2.5 rounded-xl bg-accent text-accent-foreground text-xs flex items-center justify-between">
            <span>
              <span className="font-semibold">{firstName}'s goal:</span> {targetRole} · Day {planDay}/90
            </span>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              {msg.role === "assistant" && (
                <div className="w-7 h-7 rounded-full gradient-primary flex items-center justify-center text-primary-foreground text-[10px] font-bold shrink-0 mt-1 mr-2">AI</div>
              )}
        ...
        <div className="px-4 py-3 border-t border-border shrink-0">
          <div className="flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="Talk to Remote Workher AI..."
          ...
          <p className="text-[10px] text-muted-foreground mt-1.5 text-center">Unlimited messages</p>
        </div>
      </div>
    </>
  );

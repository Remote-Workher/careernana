    ...
    <div className="w-full animate-fade-in">
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-primary text-primary-foreground text-[12.5px] font-semibold px-4 py-2.5 rounded-[10px] hover:bg-primary-dark transition-colors"
        >
      ...
      {/* Filters */}
      <div className="bg-card border border-border rounded-[14px] p-3 mb-5 flex flex-col md:flex-row gap-2.5">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2.5 rounded-[10px] border border-border bg-background text-[13px] outline-none focus:border-primary"
          />
        </div>
        <select
          value={workType}
          onChange={(e) => setWorkType(e.target.value)}
          className="px-3 py-2.5 rounded-[10px] border border-border bg-background text-[13px] outline-none focus:border-primary"
        >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filtered.map((j) => (
            <div key={j.id} className="bg-card border border-border rounded-[14px] p-4 hover:border-primary/40 transition-colors flex flex-col">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <p className="text-[14px] font-bold text-foreground truncate">{j.job_title}</p>
                  <p className="text-[12px] text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Building2 className="w-3 h-3" /> {j.company}
                  </p>
                </div>
                {j.work_type && (
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full text-primary bg-primary-tint shrink-0 capitalize">{j.work_type}</span>
                )}
              </div>
              {j.location && (
              ...
              <div className="mt-auto flex items-center gap-2 pt-2">
                <button
                  ...
                  className="flex-1 text-[11.5px] font-semibold bg-primary text-primary-foreground py-2 rounded-[8px] hover:bg-primary-dark transition-colors"
                >
                  Apply with AI
                </button>
                <a
                  ...
                  className="text-[11.5px] font-semibold text-foreground border border-border py-2 px-3 rounded-[8px] hover:bg-muted transition-colors inline-flex items-center gap-1"
                >

...
export default function SalaryAnalyzer() {
  ...
  type SalaryCache = { title: string; experience: string; city: string; workType: string };
  const cached = readToolResult<SalaryCache>("salary-analyzer");
  const [title, setTitle] = useState(cached?.title ?? "Product Designer");
  const [experience, setExperience] = useState(cached?.experience ?? "3-5");
  const [city, setCity] = useState(cached?.city ?? "Lagos");
  const [workType, setWorkType] = useState(cached?.workType ?? "Full-time");
  const [analyzed, setAnalyzed] = useState<boolean>(!!cached);
  useCachedToolResult("salary-analyzer", analyzed ? { title, experience, city, workType } : null);
          ...
          {analyzed && (
            <div className="mt-4 rounded-[14px] p-4" style={{ background: "linear-gradient(135deg, #FDF1F5, #E0F2FE)", border: "1px solid #F7CDD9" }}>
              <p className="text-[11px] leading-relaxed text-[#E0487A]">
                💡 <strong>Negotiation tip:</strong> My Wins shows quantified results. Use them in negotiation — professionals who present specific impact metrics push offers 15–20% above median.
              </p>
            ...
            <>
              ...
              <div className="rounded-[14px] p-6 text-white" style={{ background: "linear-gradient(135deg, #E0487A, #c73868)", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}>
                ...
                <p className="text-[11px] opacity-80">
                  🎯 My Wins strength puts you in the top 20% — aim for {fmt(Math.round(minSal * 1.3))}–{fmt(Math.round(median * 1.2))} minimum
                </p>
              </div>
...
Given my track record — including [insert 1–2 quantified wins from My Wins, e.g. "leading the launch of X which grew Y by Z%"] — I was hoping we could land closer to ${fmt(target)}/month. I'm open to discussing the full package: base, performance bonus, learning budget, and remote/flex setup.
